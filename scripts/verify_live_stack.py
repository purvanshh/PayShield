...
def _score_payload(txn_id: str, user_id: str, now: str) -> dict:
    ...
    return {
        ...
        "location": {"lat": 28.6139, "lon": 77.2090} if suspicious else {"lat": 19.0760, "lon": 72.8777},
        "mcc_code": "fashion",
        "txn_type": "P2M",
    }
...
async def verify(base_url: str) -> list[dict]:
    ...
    def record(name, expected, measured, ok):
        results.append({"scenario": name, "expected": expected, "measured": measured, "ok": ok})
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=30.0) as client:
        ...
        r = await client.post(
            "/v1/return/score",
            json={"order_id": "ORD_SERIAL_001", "user_id": "U_SERIAL_001",
                  "merchant_id": "M_FASHION_001", "amount": 5500, "category": "fashion",
                  "payment_method": "UPI", "cod_flag": True},
            headers=HEADERS,
        )
        if r.status_code == 200:
            d = r.json()["data"]
            ok = d["risk_tier"] == "HIGH" and 0.7 <= d["return_risk_score"] <= 0.95
            record("serial returner /v1/return/score", "HIGH ~0.83",
                   f"{d['risk_tier']} {d['return_risk_score']}", ok)
        else:
        ...
        r = await client.post(
            "/v1/return/score",
            json={"order_id": "ORD_HONEST_001", "user_id": "U_HONEST_001",
                  "merchant_id": "M_ELECTRONICS_001", "amount": 12000, "category": "electronics",
                  "payment_method": "UPI", "cod_flag": False},
            headers=HEADERS,
        )
        if r.status_code == 200:
            d = r.json()["data"]
            ...
            record("honest customer /v1/return/score", "LOW ~0.10",
                   f"{d['risk_tier']} {d['return_risk_score']}", ok)
        else:
        ...
        r = await client.post(
            ...
            json={"dispute_id": "CB_WINNABLE_001", "payment_id": "pay_CLEAN_001",
                  "transaction_id": "TXN_CLEAN_001", "network": "VISA",
                  "reason_code": "10.4", "reason_description": "Fraud - Card Not Present",
        ...
        if r.status_code == 200:
            ...
            record("winnable chargeback", "REJECT conf >=0.85",
                   f"{d['response_type']} conf {d['confidence_score']}", ok)
        else:
        ...
        r = await client.post(
            "/v1/chargeback/respond",
            json={"dispute_id": "CB_WEAK_001", "payment_id": "pay_NEW_001",
                  "transaction_id": "TXN_NEW_001", "network": "UPI",
                  "reason_code": "FRAUD", "reason_description": "Fraudulent Transaction",
        ...
        if r.status_code == 200:
            ...
            record("weak chargeback", "PARTIAL + >=2 warnings",
                   f"{d['response_type']} {len(d['warnings'])} warnings", ok)
        else:
            record("weak chargeback", "PARTIAL", f"HTTP {r.status_code}", False)

        # clean txn (ALLOW)
        now = datetime.utcnow().isoformat()
        r = await client.post("/v1/score", json=_score_payload("TXN_LIVE_CLEAN", "U_CLEAN_001", now),
                              headers=HEADERS)
        if r.status_code == 200:
            d = r.json()
            ok = d["decision"] == "ALLOW" and d["fraud_probability"] <= 0.2
            record("clean txn /v1/score", "ALLOW <=0.2", f"{d['decision']} {d['fraud_probability']}", ok)
        else:
            record("clean txn", "ALLOW", f"HTTP {r.status_code}", False)

        # suspicious burst (BLOCK)
        r = await client.post("/v1/score", json=_score_payload("TXN_LIVE_SUSP", "U_FRAUD_001", now),
                              headers=HEADERS)
        if r.status_code == 200:
            d = r.json()
            ok = d["decision"] == "BLOCK"
            record("suspicious burst", "BLOCK",
                   f"{d['decision']} {d['fraud_probability']} {d['evidence'].get('triggered_rules', [])}", ok)
        else:
        ...
        r = await client.post("/webhooks/razorpay/chargeback", content=body,
        ...
        r = await client.post(
            "/v1/return/update",
            json={"user_id": "U_HONEST_001", "order_id": "ORD_LIVE_UPDATE", "amount": 1500,
...
def main():
    ...
    widths = [max(len(headers[0]), *(len(r["scenario"]) for r in results)),
              max(len(headers[1]), *(len(str(r["expected"])) for r in results)),
              max(len(headers[2]), *(len(str(r["measured"])) for r in results)), 6]
    ...
    for r in results:
        ...
        if r["ok"]:
            passed += 1
        print(" | ".join(
            [r["scenario"].ljust(widths[0]), str(r["expected"]).ljust(widths[1]),
             str(r["measured"]).ljust(widths[2]), status.rjust(widths[3])]
        ))
