...
def test_base_scenario_reproduces_doc_headline():
    ...
    assert result["false_blocks"] == 18
    assert result["true_caught"] == 1071
    assert result["prevented"] == 750
    assert result["remaining_returns"] == 1050
    assert result["baseline_cost"] == 5_031_000
    assert result["payshield_cost"] == 2_938_350
    assert result["monthly_savings"] == 2_092_650
    assert round(result["roi_pct"], 1) == 41.6
...
def test_high_gate_uses_block_penalty():
    ...
    assert result["false_blocks"] > 0  # precision 0.9837, not 1.0
