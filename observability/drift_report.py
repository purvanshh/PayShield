...
(value -> timestamp). This module compares yesterday's distribution
against today's per feature using the Population Stability Index.
"""
    ...
    for key in FEATURE_KEYS:
        try:
            ...
            if expected.var() == 0 and actual.var() == 0 and float(expected[0]) == float(actual[0]):
                features[key] = {"psi": 0.0, "status": "STABLE",
                                 "expected_samples": int(expected.size), "actual_samples": int(actual.size)}
                continue
            psi = population_stability_index(expected, actual)
            if not math.isfinite(psi):
                features[key] = {"psi": None, "status": "CONSTANT",
                                 "expected_samples": int(expected.size), "actual_samples": int(actual.size)}
                continue
            features[key] = {
                "psi": round(psi, 4),
