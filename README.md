...
**Run it (hermetic, no services):** `python scripts/train_xgb_return_risk.py`

**Honest prototype note:** a student PoC on Razorpay's infrastructure — not
production software. Fraud detection and chargeback response ship as platform
extensions on the same audit chain.
...
|----------|--------|------------------|
| **Offline XGBoost** | **0.8067** | Model learns from raw 7 features + hidden DGP noise (architecture validation) |
| **Live Redis-backed** | **0.9311** | Real user history, merchant baselines, device fingerprints enrich the features (production path) |
| **Gap** | **+0.12** | Feature engineering (Redis enrichment) matters as much as model choice |

**Why two numbers?** The offline pipeline validates the *model architecture*;
the live pipeline validates the *production system*. The gap between them is the
highest-leverage investment — more than model tuning, more than algorithm choice.

### Offline validation — a trained model (raw features)

The checkout-time scorer runs an **XGBoost model** (tuned over 144
hyperparameter combinations) as its primary engine. On a raw 10,000-order
generator hold-out (per-user chronological, no future data), base rate ~40%:
...
both naive rules (**+0.11 over the best naive baseline**). The hand-tuned logic
was already close to optimal on the linear part of the signal; XGBoost's edge is
the nonlinear interactions, and it is the architecture that scales with enriched
features.

**Honest limitation — the data generating process:**

The synthetic return labels are generated by a logistic model that includes
**both visible features (the 7 XGBoost sees) and hidden features** (product
rating, delivery speed, packaging quality, weather delays, customer mood) that
the model never observes. This means:

1. **XGBoost learns from noisy, incomplete signal** — like real merchant data.
2. **The ablation drops measure genuine feature importance** against hidden confounders.
3. **Absolute PR-AUC is lower but more honest** than a circular DGP that labels
   with exactly the features the model trains on.
4. **The +0.017 lift over hand-weighted is real** — XGBoost captures nonlinear
   interactions and partial signal that fixed linear weights miss.

The live Redis-backed system (0.9311) enriches features with real user history,
which partially substitutes for the hidden features in this prototype. Real
merchant data would close the gap further.

### Live system (Redis-enriched features)

The production Redis-backed path enriches the same features with **real user
...
The ₹3.4L/month gap between the paths is feature enrichment, not model choice —
the same enrichment story as the PR-AUC gap. The config-driven gate sweep is the
...
The checkout-time scorer runs an **XGBoost model** as its primary engine with
the hand-weighted composite as an automatic fallback. The model is trained on a
synthetic population calibrated to Indian e-commerce return distributions (base
rate ~40%, ₹74.5k AOV) using a **per-user chronological 60/20/20 hold-out** so
no score ever uses future returns.

The synthetic data engine (`data/synthetic/return_risk_generator.py`) draws
each order's return label from a logistic model of the seven *visible* features
**plus hidden features the model never observes** (product rating, delivery
speed, packaging quality, weather delays, customer mood) **plus** label noise.
This deliberately **breaks the circular data-generating process**: XGBoost must
learn from noisy, incomplete signal, exactly as it would on real merchant data
where not every variable is observed.
...
**Every score is attributable**: the response carries `engine`,
`feature_importance` and the raw model inputs (`xgb_features`) alongside the
transparent feature breakdown.

**Feature importance** (tuned model, test set): `user_return_rate_90d` 0.24 ·
`user_return_rate_30d` 0.23 · `payment_method_risk` 0.20 · `amount_vs_user_aov_ratio` 0.12 ·
`device_fingerprint_match` 0.11 · `category_return_baseline` 0.05 · `days_since_last_order` 0.04.

**Ablation study** — leave-one-feature-out retraining on an independent seed-99
test set. Every feature shows a **positive, non-trivial drop against hidden
confounders** — this is genuine feature importance, not circular recovery:
...
user-history signal, so retraining without one lets the other carry it. Removing
**both** at once costs **−10.5%**, the single largest block of signal in the
model.

**Hyperparameter tuning** — exhaustive grid search over 144 combinations
(`max_depth` × `n_estimators` × `learning_rate` × `scale_pos_weight`), selected
on the validation split: best `max_depth=3, n_estimators=200,
learning_rate=0.05, scale_pos_weight=1.5` → test PR-AUC **0.8067**.
...
**Production path:** XGBoost primary → hand-weighted fallback if the model is
unavailable. **Offline validation:** XGBoost on raw 7 features (PR-AUC 0.8067).
**Live enrichment:** Redis-backed features push the production path to
PR-AUC 0.9311.

The score is **fully transparent**: every feature carries a value, a normalised value, a weight and a contribution, plus a `source` tag (`redis_hash`, `computed`, `lookup_table`, `default_new_user`) — so a score can be explained down to the penny, and degraded data is visible, never hidden. The API response also names the engine that produced the score (`engine: xgboost | hand_weighted`) and the model's feature importance. Weights come from `configs/feature_registry_return.yaml`, tiers/rules from `configs/return_risk_rules.yaml` (reloadable without a deploy).
