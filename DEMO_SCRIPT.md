...
# [FIX] Pre-warm an LLM investigation (~60s before recording; CPU LLM takes ~35s).
# Run this burst loop once with PREWARM_ ids, then record the investigation txn id:
for i in 00 02 04 06 08 10 12 14 16 18 20 22 24 26; do
  curl -s -X POST http://localhost:8000/v1/score \
    ...
    -d "{\"txn_id\":\"PREWARM_$i\",\"user_id\":\"U_PREWARM\",\"merchant_id\":\"M_JEWEL\",
      \"amount\":95000,\"timestamp\":\"2026-08-01T10:30:$i\",\"device_fingerprint\":\"DEV_PREWARM\",
...
**T2 — burst loop (14 rapid ₹95k txns, 2s apart):**
```bash
for i in 00 02 04 06 08 10 12 14 16 18 20 22 24 26; do
  curl -s -X POST http://localhost:8000/v1/score \
    -H "X-API-Key: payshield-dev-key-2026" -H "Content-Type: application/json" \
    -d "{\"txn_id\":\"DEMO_BURST_$i\",\"user_id\":\"U_BURST\",\"merchant_id\":\"M_JEWEL\",
...
```
**Show:** txns 6–14 → `decision: "REVIEW"`, `triggered_rules: ["V-RULE-03"]`.
...
```
**Show:** `decision: "BLOCK"`, `fraud_probability: 1.0`, `triggered_rules: ["G-RULE-01"]`.
**Caption:** *"Geo-velocity. Mumbai to Delhi in 10 minutes — 3,400 km/h. BLOCK."*
**[FIX]** `G-RULE-02` is `ESCALATE`, not BLOCK — the BLOCK comes from `G-RULE-01`
(velocity > 900 km/h).

### 1:00 – 1:20 | Dashboard + Investigation
**Browser (localhost:3000, already logged in).** The alert toast for the burst
REVIEW txns should appear (live websocket `fraud_alert`). Click the last
`DEMO_BURST_26` investigation in the list, or open the detail page.

**T2 — investigation (pre-warmed `PREWARM_26` is guaranteed ready; the
`DEMO_BURST_26` one may need +35s):**
```bash
...
PSI, and use that number in the caption. (Rehearsal run 2026-08-01: **3.70**.)
...
- [ ] Drift caption number matches a rehearsal run (not the 3.70 above if it changed)
